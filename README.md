...
### Step 2: APIキーを設定する（推奨: `.env`）

このアプリは起動時に `.env` の `GOOGLE_MAPS_API_KEY` を自動で読み込みます。

```env
GOOGLE_MAPS_API_KEY=あなたのAPIキー
```

`app.js` 先頭の定数はプレースホルダーのままでも動作します:
...
補足:
- 一時的にコンソールから設定する場合: `window.GOOGLE_MAPS_API_KEY = '...'`
- `file://` 直開きでは `.env` を読めないため、HTTPサーバー経由で開いてください
